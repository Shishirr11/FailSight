...
def scrape_failory(deep: bool = True) -> list[dict]:
    ...
    while True:
        url = FAILORY_CEMETERY_URL if page == 1 else f"{FAILORY_CEMETERY_URL}?page={page}"
        page_results,next_url = _scrape_failory_list_page(url)
        if not page_results:
            break
        results.extend(new_records)
        logger.info(f"  Page {page}: {len(page_results)} records "
                    f"(total so far: {len(results)})")
    ...
    logger.info(f"Failory list scrape done: {len(results)} total entries.")
            ...
            if detail:
                ...
                if detail.get("full_article"):
                    ...
                    if not entry.get("key_lesson"):
                        ...
                        if paras:
                            entry["key_lesson"] = paras[-1][:500]
                    richer_reasons = _extract_reasons_from_text(
                        detail["full_article"]
                    )
                    if richer_reasons != ["unknown"]:
                        entry["failure_reasons"] = richer_reasons
                if detail.get("exact_funding_usd") is not None:
                    entry["funding_raised_usd"] = detail["exact_funding_usd"]
                for k in ("num_founders", "num_employees",

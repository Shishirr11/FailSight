...
TFIDF_MAT_PATH  = INDEX_DIR / "tfidf_matrix.npy"
...
def search(
    ...
    if fused:
        ...
        batch  = ranked_ids[offset: offset + limit + 50]
        rows   = []
        seen   = set()
        ...
        for rid in batch:
            ...
            row = con.execute(f"""
                ...
                WHERE e.record_id = ? {source_filter}
            """, [rid] + source_params).fetchdf()
